...
import { getStudentBadgesFn } from "@/server/api/student";
    ...
    <>
            ...
            onClick={() => {
              ...
              if (found)
                toast.success("Credential verified ✓", {
                  description: `${found.title} · issued ${found.issued} to Aarav Sharma.`,
                });
              else
      ...
      {open && (
        ...
        >
          ...
          >
            <div className="relative border-b border-slate-100 bg-gradient-to-br from-indigo-50 via-white to-teal-50 px-10 py-10 text-center">
              ...
              <p className="mt-3 text-sm text-slate-500">awarded to</p>
              <p className="font-display mt-1 text-xl font-semibold text-slate-900">Aarav Sharma</p>
              <p className="mt-3 text-xs text-slate-500">
                Grade 8A · Greenfield International School · Issued {open.issued}
              </p>
              ...
              <div className="mt-6 flex items-center justify-center gap-2 text-xs text-emerald-700">
                <BadgeCheck className="h-4 w-4" /> Verified credential · {open.credential}
              </div>
            </div>
            <div className="flex flex-wrap items-center justify-between gap-2 px-5 py-4">
              <p className="text-xs text-slate-500">Issued by {open.issuer}</p>
              <div className="flex gap-2">
                <button
                  onClick={() =>
                    toast.success("Download started", { description: `${open.credential}.pdf` })
                  }
                  className="inline-flex h-9 items-center gap-2 rounded-lg bg-indigo-600 px-3.5 text-xs font-semibold text-white hover:bg-indigo-700"
                >
                  <Download className="h-3.5 w-3.5" /> Download PDF
                </button>
                <button
                  onClick={() => toast.success("Share link copied to clipboard")}
                  className="inline-flex h-9 items-center gap-2 rounded-lg border border-slate-200 px-3.5 text-xs font-medium text-slate-700 hover:bg-slate-50"
                >
                  <Share2 className="h-3.5 w-3.5" /> Share
                </button>
                <button
                  onClick={() => setOpen(null)}
                  className="h-9 rounded-lg px-3 text-xs text-slate-500 hover:bg-slate-50"
                >
                  Close
                </button>
              </div>
